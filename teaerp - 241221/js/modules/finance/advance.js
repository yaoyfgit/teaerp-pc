...
function renderAdvanceTable(advances) {
    ...
    tbody.innerHTML = advances.map(advance => `
        <tr>
            ...
            <td>${advance.id}</td>
            <td>${advance.customer}</td>
            <td>${formatAmount(advance.amount)}</td>
            <td>${getReceiptMethod(advance.method)}</td>
            <td>${advance.date}</td>
...
function batchConfirm() {
    ...
    if (selectedIds.length === 0) {
        showMessage('请选择要确认的预收账款', 'error');
        return;
    }

    if (confirm(`确定要确认选中的${selectedIds.length}笔预收账款吗？`)) {
        // TODO: 调用API批量确认
...
function deleteAdvance(id) {
    if (confirm('确定要删除该预收账款吗？')) {
        // TODO: 调用API删除预收账款
        console.log('删除预收账款:', id);
        loadAdvances();
        showMessage('预收账款已删除');
    }
}
...
function viewAdvance(id) {
    // TODO: 实现查看功能
    console.log('查看预收账款:', id);
}

// 编辑预收账款
function editAdvance(id) {
    // TODO: 实现编辑功能
    console.log('编辑预收账款:', id);
}
